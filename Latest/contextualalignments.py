...
if LOCAL_MODE:
    ...
    app = Flask(__name__,
    	static_url_path='/CALVI/static',
    	static_folder='/home/calvi/static')
#########################################################################################################################################################################################################################
...
def idgetter(alignmentfile):
    ...
    return idcontainer
# ---------------------------------------------------------------------------------------------------------------------------------------------
def eggnogger(test_unip):
    if LOCAL_MODE:
        lookuppath = "/mnt/c/Users/TSchm/Desktop/Coding/Projekt 3 - Alignments/static/eggnogger_lookup.txt"
    else:
        lookuppath = "/home/calvi/static/calvi_eggnogger_lookup.txt"
    f = open(lookuppath, "r")
    ff = f.read()
    finaldict = ast.literal_eval(ff)
    targetpath = finaldict[test_unip]
    alignments={}
    with gzip.open(targetpath,"r") as alignfile:
        for oldline in alignfile:
            ...
            if ">" in line:
                ...
                if line.replace(" ","").replace("\n","") != "":
                    alignments[seq_name]+=line.replace(" ","").replace("\n","")
    if LOCAL_MODE:
        translatefile = "/mnt/c/Users/TSchm/Desktop/Coding/Projekt 3 - Alignments/static/Blast_Unip_AC_file_slim.txt.gz"
    else:
        translatefile = "/home/calvi/static/Blast_Unip_AC_file_slim.txt.gz"
    alignments_transl = {}
    with gzip.open(translatefile, "rt", encoding="utf-8") as aliasfile:	### slimmed down file, ~ 0.1 GB zipped
        for line in aliasfile:
            fields = line.strip().split("\t")
            if len(fields) < 2:
                continue
            interrogated_id = fields[0].replace(" ", "")
            unip_identifier = fields[1].replace(" ", "")
            if interrogated_id in alignments:
                if unip_identifier not in alignments_transl:
                    alignments_transl[unip_identifier]=alignments[interrogated_id]
    return alignments_transl
# ---------------------------------------------------------------------------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
def SHOWORDER(seqs, doi, starti, endi, goi, maxnumber, sortmech):
    # This function sorts the sequences (keys in doi) based on the count of
    # annotations in the viewing range [starti, endi]. Instead of counting
    # all features, we now count only those features that belong to the
    # selected category (unless sortmech is "FeatureSum", in which case we count all).

    showtime = {}
    for k in doi:  # For each uniprot ID
        featurecount = []
        sequenzler = seqs[k]
        residue = 0
        for i, letter in enumerate(sequenzler, start=1):
            if letter not in gapletters:
                residue += 1
                if starti <= i <= endi:
                    # Loop over the feature categories for this uniprot ID
                    for v in doi[k]:
                        # If sortmech is not "FeatureSum", count only the specified category.
                        # Use a case-insensitive comparison.
                        if sortmech != "FeatureSum" and v.lower() != sortmech.lower():
                            continue
                        for vv in doi[k][v]:
                            try:
                                if int(vv) == residue:
                                    if int(vv) not in featurecount:
                                        featurecount.append(int(vv))
                            except Exception as e:
                                pass
        if k != goi:
            showtime[k] = len(featurecount)
    # Sort by descending feature count; if equal, sort lexicographically by key.
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
def create_svg(sequences_dict, positions, colordict, startposition, windowsize, poi, forbidden, proteinfeatures, Konserve, featurecolors, translator, topgun, sortmechanism):
    heatmapper = {}
    ...
    filename 		= poi+"_Position"+str(startposition)+"_Windowsize"+str(windowsize)+"_Sequences"+str(topgun)+"_"+str(sortmechanism)+".svg"
    filename_print 	= poi+"_Position"+str(startposition)+"_Windowsize"+str(windowsize)+"_Sequences"+str(topgun)+"_"+str(sortmechanism)+"_print.svg"
    ...
    #dwg = open(filename,"a")
        ...
        roworder = SHOWORDER(sequences_dict, positions, distance_start, distance_end, poi, topgun, sortmechanism)
...
#########################################################################################################################################################################################################################
########################################################################################################################
@app.route(f'{os.getenv("URL_PREFIX", "")}/')
def index():
    return render_template("index.html", URL_PREFIX=os.getenv("URL_PREFIX", ""))

if LOCAL_MODE:
    RESULT_DIR = WURZELDIR
else:
    RESULT_DIR = '/home/calvi'

default_alignment = {}
@app.route(f'{URL_PREFIX}/generate', methods=['POST'])
def generate_result():
    ...
    sortmechanism = data.get("sortmech")
    # flag to determine if default alignment was used
    default_used = str(data.get("defaultAlignmentUsed", "false")).lower() == "true"

    if not input1 or not input2 or not topguns or not alignmentfile:
        return jsonify({"error": "All fields are required!"}), 400

    if LOCAL_MODE:
        accessionfile = "/mnt/c/Users/TSchm/Desktop/Coding/Projekt 3 - Alignments/static/accessions_lookup.txt"
    else:
        accessionfile = "/home/calvi/static/accessions_lookup.txt"
    rawf = open(accessionfile, "r")
    rawff = rawf.read()
    accdict = ast.literal_eval(rawff)
    try:
        correct_id = accdict[input1]
        input1  = correct_id
    except:
    ...
    filename = f"{input1}_Position{input2}_Windowsize{slider}_Sequences{topguns}_{sortmechanism}.svg"
    filename_print = f"{input1}_Position{input2}_Windowsize{slider}_Sequences{topguns}_{sortmechanism}_print.svg"

    if default_used and alignmentfile:
        try:
            # alignmentfile should be a JSON string if it's the default alignment
            new_alignfile = json.loads(alignmentfile)
            ids = list(new_alignfile.keys())
            sequences = new_alignfile
        except json.JSONDecodeError as e:
            return jsonify({"error": f"Invalid alignment format: {e}"}), 400
    else:
    ...
    ############
    os.chdir(RESULT_DIR)
    resultfile = create_svg(sequences, positions, colors, position_of_interest, window, protein_of_interest, TheForbiddenPositions, feature_dict, Konserve, featurecolors, translator, topguns, sortmechanism)
	#RESULT_DIR_two = os.path.join(os.getcwd(), 'static', 'results')
    if LOCAL_MODE:
        RESULT_DIR_two = os.path.join('static', 'results')
    else:
        RESULT_DIR_two = os.path.join('/home/calvi', 'static', 'results')
    ...
    #print("Looking for source file at:", temp_file_path)
...
@app.route(f'{URL_PREFIX}/download/<filename>')
def download_file(filename):
    if LOCAL_MODE:
        RESULT_DIR = os.path.join(os.getcwd(), "static","results")
    else:
        RESULT_DIR = '/home/calvi/static/results'
    return send_from_directory(RESULT_DIR, filename, as_attachment=True)


@app.route(f'{URL_PREFIX}/example_data', methods=['GET'])
def get_example_data():
    # Define the file paths for the example data
    if LOCAL_MODE:
        example_clustal_path = os.path.join(os.path.dirname(__file__), 'static', 'example', 'RHOA_BlastpExample_ClustalMSA.clustal')
    else:
        example_clustal_path = '/home/calvi/static/example/RHOA_BlastpExample_ClustalMSA.clustal'
    try:
        ...
        return jsonify({
            ...
            'svgPath': f'{URL_PREFIX}/static/example/P61586_Position13_Windowsize10.svg',
            'svgPrintPath': f'{URL_PREFIX}/static/example/P61586_Position13_Windowsize10_print.svg'
        })
...
@app.route(f'{URL_PREFIX}/robots.txt')
def robots():
    return send_from_directory(app.static_folder, "robots.txt")

@app.route(f'{URL_PREFIX}/check_alignment/<accession>')
def check_alignment(accession):
    try:
        default_alignment = eggnogger(accession)
    except Exception as e:
        #app.logger.error(f"Error in eggnogger: {e}")
        default_alignment = {}

    if len(default_alignment) >= 2:
        return jsonify({
            "message": f"Default alignment for {accession} found.",
            "alignment": default_alignment,
            "default_used": True  # Indicate that this is a default/pre-formatted alignment
        })
    else:
        return jsonify({
            "message": f"No default alignment for {accession} found.",
            "alignment": "",
            "default_used": False
        })

        
@app.route(f"{URL_PREFIX}/documentation")
def documentation():
